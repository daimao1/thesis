...
const saltRounds = 5; //bcrypt hash param
    ...
    passport.use(
        ...
        new LocalStrategy({
            ...
            function(req, email, password, done) {
                ...
                dbconnection.query("SELECT * FROM " + db.users_table + " WHERE email = '" + email + "'", function(err, rows) {
                    if (err)
                        return done(err);
                    if (rows.length) {
                        ...
                        var insertQuery = "INSERT INTO " + db.users_table + " ( `email`, `password` ) values (?,?)";
